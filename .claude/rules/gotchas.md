...
- **Auth naming:** `auth-session.ts` sets JWT cookies — not a server-side session store.
- **Production rewrite env:** `next.config.js` uses `DATABASE_HOST`, not `API_URL`.
...
- **Workspace deps first:** UI → `@repo/ui`; DB → `@repo/db` (backend only); web data → TanStack Query; web forms → TanStack Form — see [workspace-dependencies.md](workspace-dependencies.md).
...
- **Timesheets:** Backend routes under `apps/backend/app/timesheet/` — verify with `find` before assuming scope.
...
- **Turbo config:** File is `turbo.json` — declare env vars in `global.env`.
